...
*/
        ...
        public async Task<MemoryStream> ConvertByteArrayToStreamAsync(byte[] audioDataLeft, byte[] audioDataRight = null)
        {
            ...
            /*
            using (var inputStream = new MemoryStream(audioDataLeft))
            {
                var arguments = FFMpegArguments
                    .FromPipeInput(new StreamPipeSource(inputStream))
                    .OutputToPipe(new StreamPipeSink(outputStream), options => ConfigureAudioOptions(options));

                if (audioDataRight != null)
                {
                    using (var rightStream = new MemoryStream(audioDataRight))
                    {
                        arguments.AddPipeInput(new StreamPipeSource(rightStream));
                        arguments.WithCustomArgument("-filter_complex \"[0:a][1:a]amerge=inputs=2[aout]\" -map \"[aout]\"");
                    }
                }
